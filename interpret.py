...
from pretrain_predict import make_predictions
from utils import load_args, load_checkpoint, timeit
from data import MoleculeDataLoader, MoleculeDataset, MoleculeDatapoint

MIN_ATOMS = 15
C_PUCT = 10


def prepare_data_for_interpret(smiles:List[str],features_generator:List[str] = None, batch_size: int =128)-> MoleculeDataLoader:
      
    data = MoleculeDataset([MoleculeDatapoint(smiles=smile,features_generator=features_generator) for smile in smiles])
    original_data_len = len(data)
    data = MoleculeDataset([datapoint for datapoint in tqdm(data)
    ...
    loader = MoleculeDataLoader(dataset=data,batch_size=batch_size)

    return loader

class RetentionOrderRP:
    """A :class:`RetentionOrderRP` is a wrapper around a :class:`~model.MoleculeModel` for interpretation."""

    def __init__(self, args: InterpretArgs) -> None:
        """
        :param args: A :class:`~RetentionOrderRP.args.InterpretArgs` object containing arguments for interpretation.
        """
        self.args = args
    ...
    def __call__(self, smiles: List[str],batch_size: int = 128) -> List[List[float]]:
        """
        ...
        """
        test_data_loader = prepare_data_for_interpret(smiles = smiles,features_generator = self.args.features_generator,batch_size = self.args.batch_size)

        sum_preds = []
        for model in self.checkpoints:
            model_preds = make_predictions(model=model, data_loader=test_data_loader, disable_progress_bar=True) 
...
def mcts_rollout(node: MCTSNode,
    ...
    :param scoring_function: A function for scoring subgraph SMILES using a RetentionOrderRP model.
...
def interpret(args: InterpretArgs) -> None:
    """
    Runs interpretation of a RetentionOrderRP model using the Monte Carlo Tree Search algorithm.
    """
    
    global C_PUCT, MIN_ATOMS

    ROSMI = RetentionOrderRP(args)

    def scoring_function(smiles: List[str]) -> List[float]:
        return ROSMI(smiles)[:,0]

    C_PUCT = args.c_puct
    MIN_ATOMS = args.min_atoms

    with open(args.interpret_path) as f:
        read = csv.DictReader(f)
        all_smiles = [[row[c] for c in ['SMILES']] for row in read]

    print(f'smiles,RT,rationale,rationale_score')
    
    new_smiles,new_score,new_rats_smiles,new_rats_P = [],[],[],[]
    ...
    for smiles in all_smiles:
        ...
        if len(rationales) == 0:
            ...
            new_rats_smiles.append(rats[0].smiles)#[]
            new_rats_P.append(rats[0].P)#[]
    new_smiles = [num for elem in new_smiles for num in elem] #嵌套列表的平铺
    #pred_2 = [num for elem in pred_2 for num in elem]
    save_preds_dataframe = pd.DataFrame(data={'smiles': new_smiles,'RT': new_score,'rationale':new_rats_smiles,'rationale_score': new_rats_P})
    save_preds_dataframe.to_csv(args.SAVE_PTAH_NAME, index=False)

            
def ROSMI_interpret() -> None:
    """Runs interpretation of a RetentionOrderRP model.

    This is the entry point for the command line command :code:`ROSMI_interpret`.
    """
